 ...
 */
/* jshint -W062 */
/* global MiluParams, WOW, AOS, WVC, Cookies, Event, objectFitImages, WVCBigText, WVCParams, MiluYTVideoBg, Vimeo */
...
var MiluUi = function( $ ) {
	...
	return {
		...
		setClasses : function () {
			...
			if ( ( this.isMobile || 800 > $( window ).width() ) && ! MiluParams.forceAnimationMobile ) {
		...
		loadingAnimation : function () {
			...
			if ( ! MiluParams.defaultPageLoadingAnimation ) {
				return false;
			}
			...
			if ( $.isFunction( $.fn.stick_in_parent ) ) {
				if ( $( 'body' ).hasClass( 'single-work-layout-sidebar-left' ) || $( 'body' ).hasClass( 'single-work-layout-sidebar-right' ) ) {
					$( '.work-info-container' ).stick_in_parent( {
						offset_top : parseInt( MiluParams.portfolioSidebarOffsetTop, 10 )
					} );
				}
			}
		},
		...
		mobileMenuBreakPoint : function () {
			...
			var $body = $( 'body' ),
				...
				breakpoint = MiluParams.breakPoint;
			...
			if ( breakpoint > winWidth ) {
				...
				$( window ).trigger( 'milu_breakpoint' );
		...
		subMenuDirection : function() {
			...
			var $this,
				subMenuWidth = parseInt( MiluParams.subMenuWidth, 10 ),
				bleed = 8;
		...
		centeredLogo : function () {
			...
			$( '<li class="logo-menu-item">' + MiluParams.logoMarkup + '</li>' ).insertAfter( '#site-navigation-primary-desktop > li:nth-child(' + middleItemCount + ')' );
		},
		...
		stickyMenu : function ( scrollTop ) {
			...
			var scrollPoint,
				menuOffset = parseInt( MiluParams.menuOffset, 10 );

			scrollTop = scrollTop || 0;

			scrollPoint = parseInt( MiluParams.stickyMenuScrollPoint, 10 );
			...
			if ( menuOffset ) {
				...
				if ( '100%' === MiluParams.menuOffset ) {

					scrollPoint = $( window ).height() - parseInt( MiluParams.desktopMenuHeight, 10 );

				} else {
					scrollPoint = menuOffset - parseInt( MiluParams.desktopMenuHeight, 10 );
				}
			}

			if ( 'soft' === MiluParams.stickyMenuType ) {
			...
			} else if ( 'hard' === MiluParams.stickyMenuType ) {
		...
		parallax : function () {
			...
			var smallScreen = ( ( 800 > $( window ).width() || this.isMobile ) && MiluParams.parallaxNoSmallScreen );

			if ( ! smallScreen ) {
				$( '.parallax' ).jarallax( {
					noAndroid : MiluParams.parallaxNoAndroid,
					noIos : MiluParams.parallaxNoIos
				} );
			}
		},
		...
		toggleSearchForm : function () {
			$( document ).on( 'click', '.toggle-search', function() {
				$( window ).trigger( 'milu_searchform_toggle' );
		...
		flexSlider : function () {
			...
			if ( $.isFunction( $.flexslider ) ) {
				...
				$( '.entry-slider' ).flexslider( {
					animation: MiluParams.entrySliderAnimation,
					slideshow : true,
					slideshowSpeed : 4000
				} );
			}
		...
		lightbox : function () {
			...
			if ( 'fancybox' === MiluParams.lightbox ) {

				$( selectors ).fancybox( MiluParams.fancyboxSettings );
				...
				$( '.gallery-quickview, .wvc-gallery-quickview' ).unbind().on( 'click', function() {
					...
					params = MiluParams.fancyboxSettings;
				...
				$( '.woocommerce-product-gallery__trigger' ).unbind().on( 'click', function() {
					...
					$.fancybox.open( quickviewData, MiluParams.fancyboxSettings );
					return false;
				} );
				...
				$( '.lightbox-video' ).fancybox( {
					...
					afterLoad : function() {
						...
						MiluUi.lightboxVideoAfterLoad();
					},

					afterClose : function() {

						MiluUi.restartVideoBackgrounds();
					}
				} );
			}
		},
		...
		lightboxVideoAfterLoad : function () {
			...
			var $iframe = $( '.fancybox-iframe' ).contents(),
				...
				accentColor = MiluParams.accentColor;
			...
			if ( $video.length ) {
				...
				$head.append( $( '<link/>',
					{
						rel: 'stylesheet',
						href: MiluParams.mediaelementLegacyCssUri,
						type: 'text/css'
					} )
				);
				$head.append( $( '<link/>',
				...
				$head.append( $( '<link/>',
					{
						rel: 'stylesheet',
						href: MiluParams.fancyboxMediaelementCssUri,
						type: 'text/css'
					} )
				);
		...
		scrollToMainContent : function () {
			...
			}, parseInt( MiluParams.smoothScrollSpeed, 10 ), MiluParams.smoothScrollEase, function() {
		...
		smoothScroll : function( href ) {
			...
			if ( -1 !== href.indexOf( '#' ) ) {
				...
				if ( $( '#' + hash ).length ) {
					...
					if ( 'hard' === MiluParams.stickyMenuType && ! $( 'body' ).hasClass( 'sticky-menu-transparent' ) && ! $( '#' + hash ).hasClass( 'wvc-row-full-height' ) ) {
						scrollOffset += parseFloat( MiluParams.stickyMenuHeight, 10 );
					}
		...
		wowAnimate : function () {
			...
			var wowAnimate,
				doWow = ( MiluParams.forceAnimationMobile || ( ! this.isMobile && 800 < $( window ).width() ) );

			if ( doWow && 'undefined' !==  typeof WOW ) {
				wowAnimate = new WOW( { offset : MiluParams.WOWAnimationOffset } ); // init wow for CSS animation
				wowAnimate.init();
			}
		},
		...
		liveSearch : function () {
			...
			$( '.live-search-form' ).each( function() {
				var $formContainer = $( this ),
					...
					timer = null,
					$resultContainer,
					action = 'milu_ajax_live_search',
					result;

				if ( $form.hasClass( 'woocommerce-product-search' ) ) {
					action = 'milu_ajax_woocommerce_live_search';
				}
		...
		setInternalLinkClass : function () {
			...
			var siteURL = MiluParams.siteUrl,
				$internalLinks,
				regEx = '';

			$.each( MiluParams.allowedMimeTypes, function( index, value ) {
				regEx += '|' + value;
			} );

			regEx = $.trim( regEx ).substring(1);

			siteURL = MiluParams.siteUrl;
			...
			if ( MiluParams.isWooCommerce ) {

				/*
				When WC pages aren't set the WC pages variables will return the siteURL
				Be sure it is not the same !!
				 */
				if ( this.untrailingSlashit( siteURL ) !== this.untrailingSlashit( MiluParams.WooCommerceCartUrl ) ) {
					$( 'a[href^="' + MiluParams.WooCommerceCartUrl + '"]' ).removeClass( 'internal-link' );
				}

				if ( this.untrailingSlashit( siteURL ) !== this.untrailingSlashit( MiluParams.WooCommerceAccountUrl ) ) {
					$( 'a[href^="' + MiluParams.WooCommerceAccountUrl + '"]' ).removeClass( 'internal-link' );
				}

				if ( this.untrailingSlashit( siteURL ) !== this.untrailingSlashit( MiluParams.WooCommerceCheckoutUrl ) ) {
					$( 'a[href^="' + MiluParams.WooCommerceCheckoutUrl + '"]' ).removeClass( 'internal-link' );
				}
		...
		transitionCosmetic : function() {
			...
			if ( ! MiluParams.defaultPageTransitionAnimation ) {
				return false;
			}

			var _this = this;

			if ( MiluParams.isAjaxNav ) {
				return;
			}
			...
			$( document ).on( 'click', '.internal-link:not(.disabled)', function( event ) {
				...
				if ( ! event.ctrlKey ) {
					...
					if ( MiluParams.hasLoadingOverlay ) {
		...
		videoThumbnailPlayOnHover : function() {
			...
			$( 'iframe.youtube-bg', itemsContainer ).each( function() {
				if ( 'undefined' !== typeof MiluYTVideoBg && MiluYTVideoBg.players[ $( this ).data( 'youtube-video-id' ) ] ) {
					MiluYTVideoBg.players[$( this ).data( 'youtube-video-id' )].pauseVideo();
				}
			} );
				...
				if ( $iframe.length ) {
					...
					if ( $iframe.hasClass( 'youtube-bg' ) ) {
						...
						$( this ).on( 'mouseenter', function() {
							...
							if ( 'undefined' !== typeof MiluYTVideoBg && MiluYTVideoBg.players[ YTPlayerId ] ) {
								MiluYTVideoBg.players[YTPlayerId].playVideo();
							}
						} );
						...
						$( this ).on( 'mouseleave', function() {
							...
							setTimeout( function() {
								if ( 'undefined' !== typeof MiluYTVideoBg && MiluYTVideoBg.players[ YTPlayerId ] ) {
									MiluYTVideoBg.players[YTPlayerId].pauseVideo();
								}
							}, 500 );
						} );
		...
		navPlayer : function () {
			$( document ).on( 'click', '.nav-play-button', function() {
				...
				var $btn = $( this ),
					...
					audioId = $audio.attr( 'id' ),
					audio = document.getElementById( audioId ),
					playText = MiluParams.l10n.playText,
					pauseText = MiluParams.l10n.pauseText;
		...
		pageLoad : function () {
			...
			delay = MiluParams.pageTransitionDelayAfter;
			...
			}, parseInt( MiluParams.pageLoadedDelay, 10 ) );
		}
	};
...
( function( $ ) {
	...
	if ( 'undefined' !== typeof WVC ) {
		 ...
		 */
		WVC.lightbox = MiluUi.lightbox;

		/**
		 * Overwrite WVC video opener
		 */
		WVC.videoOpener = MiluUi.videoOpener;

		/**
		 * Overwrite toolbar offset calculation
		 */
		WVC.getToolBarOffset = MiluUi.getToolBarOffset;
	}
