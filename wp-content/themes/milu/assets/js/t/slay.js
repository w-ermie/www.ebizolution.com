...
var Slay = function( $ ) {
	...
	return {
		...
		cursor : function() {
			...
			$( 'body.custom-cursor-enabled' ).append( '<div id="slay-cursor-dot-holder"><span id="slay-cursor-dot"></span></div>' );

			var $dotHolder = $( '#slay-cursor-dot-holder' ),
				$dot = $( '#slay-cursor-dot' );
			...
			$( 'a' ).on( 'mouseenter', function() {
				...
				if ( $( this ).hasClass( 'slay-button-solid-accent' ) ) {
		...
		transitionCosmetic : function() {
			...
			$( document ).on( 'click', '.internal-link:not(.disabled)', function( event ) {
				...
				if ( ! event.ctrlKey ) {
					...
					if ( $( '.slay-overlay-loader-block' ).length ) {

						$( '#slay-overlay-loader-block-left' ).one( MiluUi.transitionEventEnd(), function() {
							Cookies.remove( MiluParams.themeSlug + '_session_loaded' );
							window.location = $link.attr( 'href' );
						} );

					} else if ( $( '.slay-loader-overlay' ).length ) {
						$( '.slay-loader-overlay' ).one( MiluUi.transitionEventEnd(), function() {
							Cookies.remove( MiluParams.themeSlug + '_session_loaded' );
							window.location = $link.attr( 'href' );
						} );

					} else if ( $( '.slay-loader-logo' ).length ) {
						$( '.slay-loader-logo' ).one( MiluUi.transitionEventEnd(), function() {
							Cookies.remove( MiluParams.themeSlug + '_session_loaded' );
							window.location = $link.attr( 'href' );
						} );
		...
		*/
		loadingAnimation : function () {
			...
			var _this = this,
		    	...
		    	if ( $( '#slay-percent' ).length || $( '#slay-loading-logo' ).length || $( '#slay-overlay-loader-block-left' ).length ) {
		    		return;
		    	}
