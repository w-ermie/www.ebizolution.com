	...
	return {
		...
		loadMorePosts : function () {
			...
			$( document ).on( 'click', '.loadmore-button', function( event ) {
				...
				if ( MiluParams.isCustomizer ) {
					...
					alert( MiluParams.l10n.infiniteScrollDisabledMsg );
					return;
				}
				...
				$button.html( MiluParams.l10n.infiniteScrollMsg ); // loading message
		...
		processContent : function ( response, $button ) {
			...
			if ( response ) {
				...
				if ( MiluParams.doLoadMorePaginationHashChange ) {
				...
				if ( max < nextPage || undefined === nextPage || isNaN( nextPage ) ) {
					...
					$button.html( MiluParams.l10n.infiniteScrollEndMsg );
					...
					$.post( MiluParams.ajaxUrl, {

						action: 'milu_ajax_get_next_page_link',
						href : $button.attr( 'href' )
						...
						$button.html( MiluParams.l10n.loadMoreMsg );
					} );
				}
		...
		scrollToPoint : function ( scrollPoint ) {
			...
			$( 'html, body' ).stop().animate( {
				...
				scrollTop: scrollPoint - MiluUi.getToolBarOffset()
		...
		callBack : function ( $container ) {
			...
			if ( 'undefined' !== typeof MiluUi ) {
				MiluUi.adjustmentClasses();
				MiluUi.resizeVideoBackground();
				MiluUi.lazyLoad();
				MiluUi.fluidVideos( $container, true );
				MiluUi.flexSlider();
				MiluUi.lightbox();
				MiluUi.addItemAnimationDelay();
				MiluUi.parallax();
				MiluUi.setInternalLinkClass();
				MiluUi.muteVimeoBackgrounds();

				setTimeout( function() {
					MiluUi.videoThumbnailPlayOnHover();
				}, 200 );
			}

			/* YT background */
			if ( 'undefined' !== typeof MiluYTVideoBg ) {
				MiluYTVideoBg.init( $container );
				MiluYTVideoBg.playVideo( $container );
			}

			if ( $( '.masonry-container' ).length || $( '.metro-container' ).length ) {
				MiluMasonry.masonry();
				MiluMasonry.resizeTimer();
