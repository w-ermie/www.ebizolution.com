<?php
/**
 * Milu customizer colors preview functions
 *
 * @package WordPress
 * @subpackage Milu
 * @version 1.0.2
...
/**
 * Registers color schemes for Milu.
 *
 * Can be filtered with {@see 'milu_color_schemes'}.
 *
 ...
 */
function milu_get_color_schemes() {
	/**
	 * Filter the color schemes registered for use with Milu.
	 *
	 ...
	 * }
	 */
	return apply_filters( 'milu_color_schemes', array(

		'default' => array(
			'label'  => esc_html__( 'Default', 'milu' ),
			'colors' => array(
				'#ffffff', // body_bg
				'#ffffff', // page_bg
 ...
 */
function milu_get_color_scheme_css( $colors ) {
	...
	$border_color = vsprintf( 'rgba( %s, 0.08)', milu_hex_to_rgb( $strong_text_color ) );
	...
	***********************/
	...
	return apply_filters( 'milu_color_scheme_output', $output, $colors );
}
 ...
 * @return array $colors
 */
function milu_get_template_colors() {
 ...
 * Customizer preview.
 */
function milu_color_scheme_css_template() {

	$colors = milu_get_template_colors();
	?>
	<script type="text/html" id="tmpl-milu-color-scheme">
		<?php echo milu_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'milu_color_scheme_css_template' );