...
;( function( $ ) {
	 ...
	 * Background
	 */
	var colorSchemeStyle = $( '#milu-color-scheme-css' ),
		fontsStyle = $( '#milu-fonts-css' ),
		layoutStyle = $( '#milu-layout-css' ),
		api = wp.customize,
		backgrounds = {
	...
	if ( ! colorSchemeStyle.length ) {
		colorSchemeStyle = $( 'head' ).append( '<style type="text/css" id="milu-color-scheme-css" />' )
			.find( '#milu-color-scheme-css' );
	}

	// Fonts style tag
	if ( ! fontsStyle.length ) {
		fontsStyle = $( 'head' ).append( '<style type="text/css" id="milu-fonts-css" />' )
			.find( '#milu-fonts-css' );
	}

	// Layout style tag
	if ( ! layoutStyle.length ) {
		layoutStyle = $( 'head' ).append( '<style type="text/css" id="milu-layout-css" />' )
			.find( '#milu-layout-css' );
	}
	...
	api( 'menu_width', function( value ) {
		value.bind( function( to ) {
			...
			MiluUi.centeredLogoOffset();
			MiluUi.subMenuDirection();
		} );
	} );
