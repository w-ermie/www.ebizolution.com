...
/**
    ...
    <!--            <div class="aol_value aol_add_field"><label><?php _e('Defult value', 'ApplyOnline') ?></label><input type="text" id="adapp_value" placeholder="<?php _e('Defult value') ?>" ></div>-->
                    ...
                    <div class="aol_file_types"><label for="adapp_file_types">*<?php _e('Allowed File Types', 'ApplyOnline') ?></label><input type="text" id="adapp_file_types" class="aol-form-fild adapp_file_types" value="<?php echo ALLOWED_FILE_TYPES; ?>" ></div>
        ...
        public function row_popup($key, $val, $template = NULL){ 
            ...
            $types = !empty($val['allowed_file_types']) ? ($val['allowed_file_types']) : ALLOWED_FILE_TYPES;
            ...
            if(in_array($val['type'], array('checkbox','dropdown','radio','text_area','text','number','email','date','file'))):
                if($val['type'] == 'file'){
                    ...
                    echo '<div><label for="'.$name.'-allowed-size">'.__('Max Size', 'ApplyOnline').'</label><input id="'.$name.'-allowed-size" class="aol-form-field file_size" type="text" name="'.$name.'[allowed_size]" value="'.$size.'" /></div>';
                }
                echo '<div><label for="'.$name.'-required">'.__('Required Field', 'ApplyOnline').'</label><input id="'.$name.'-required" class="aol-form-field required_option" type="checkbox" '.$checked.' name="'.$name.'[required]" value="1" /></div>';
            endif;
            ...
            <form action="options.php" method="post" name="">
                ...
                ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <textarea id="aol_allowed_file_types" name="aol_allowed_file_types" class="code" placeholder="<?php echo ALLOWED_FILE_TYPES; ?>" cols="50" rows="2"><?php echo esc_textarea(get_option_fixed('aol_allowed_file_types', 'jpg,jpeg,png,doc,docx,pdf,rtf,odt,txt')); ?></textarea>
                            <p class="description"><?php printf(__('Comma separated names of file extentions. Default: $s', 'ApplyOnline'), ALLOWED_FILE_TYPES); ?></p>
                        </td>
                    </tr>
                    <?php 
                    $settings = get_aol_settings();
